...
LeaveTimeOut = LeaveTimeOut > 600 ? 600 : LeaveTimeOut && LeaveTimeOut < 2 ? 1 : LeaveTimeOut; // prettier-ignore

MaxResponseTime = MaxResponseTime > 150 ? 150 : MaxResponseTime && MaxResponseTime < 5 ? 5 : MaxResponseTime; // prettier-ignore
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(!interaction.member.voice.channel) {
            interaction.followUp(':no_entry: Please join a voice channel and try again!');
            return;
        }
        ...
        if(isYouTubeVideoURL(query)) {
            ...
            if(player.length > maxQueueLength) {
                player.commandLock = false;
                interaction.followUp(`The queue hit its limit of ${maxQueueLength}, please wait a bit before attempting to play more songs`);
                return;
            }
            if(nextFlag || jumpFlag) {
                flagLogic(interaction, video, jumpFlag);
            ...
            YouTube.getVideo(`https://www.youtube.com/watch?v=${videos[videoIndex - 1].id}`)
                .then(function(video) {
                    ...
                    if(interaction.client.playerManager.get(interaction.guildId).queue.length > maxQueueLength) {
                        ...
                        player.commandLock = false;
                        return interaction.followUp(`The queue hit its limit of ${maxQueueLength}, please wait a bit before attempting to add more songs`);
                    }
