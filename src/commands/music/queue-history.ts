...
export const execute = async({guildId, sender, messages}: CommandInput): Promise<CommandReturn> => {
    ...
    const pageData = {title: messages.EMBED_TITLE_MUSIC_QUEUE_HISTORY(), pages: embeds, color: embedColour, author: {username: sender.user.username, avatar: sender.user.displayAvatarURL()}, listenTimeout: 2 * 60 * 1000};
